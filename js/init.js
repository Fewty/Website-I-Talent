...
function arlo_tm_animate_text(){
	...
	var animateSpan			= jQuery('.arlo_tm_animation_text_word');
		...
		animateSpan.typed({
			strings: ["Systeem- en netwerkbeheerder", "Young professional"],
			loop: true,
			startDelay: 1e3,
			backDelay: 2e3
		});
...
function arlo_tm_popup_blog_custom(){
	"use strict";
	var li				= jQuery('.arlo_tm_services_wrap .list_wrap ul li .inner');
	var popupBox		= jQuery('#arlo_tm_popup_blog');
	var popupInner		= popupBox.find('.inner_popup');
	var closePopup		= popupBox.find('.close');
	...
	li.each(function(){
		var element		= jQuery(this);
		var button		= element.find('.read_more a,.title_holder a,.link_news,.text a');
		var html		= element.html();
		var mainImage	= element.find('.news_image');
		var imgData		= mainImage.data('url');
		var title		= element.find('.title_service h3');
		var titleHref	= element.find('.title_service h3 a').html();
		...
		button.on('click',function(){
			...
			mainImage = popupInner.find('.news_image');
			mainImage.css({backgroundImage: 'url('+imgData+')'});
			title = popupInner.find('.title_service h3');
			title.html(titleHref);
			return false;
		});
	});
